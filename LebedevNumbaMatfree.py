"""
3D Elastic Wave Solver on a Lebedev grid
=========================================================================

Purpose
-------
This script uses the binary grid/cluster notation developed in the report, to solve the 3D elastic wave 
equation in a homogeneous medium using the Lebedev scheme. This implementation uses 3D
...
# 3D Lebedev elastodynamics, array-based and Numba-accelerated
...
def build_explosive_source2( 
    Nx: int, Ny: int, Nz: int,
    Lx: float, Ly: float, Lz: float,
    width: float,
    ):
    """
    builds a source with g_zz=-g_xx = g(t) for each stress cluster
    """
    x0, y0, z0 = Lx / 2.0, Ly / 2.0, Lz / 2.0
...
def apply_material_grid_major(strain_by_grid, C):
    """
    Applies sigma_rhs = C * strain_rhs on each material grid
    """
    stress_rhs_by_grid = {}

    for g in MATERIAL_GRIDS:
        exx, eyy, ezz, eyz, exz, exy = strain_by_grid[g]
        stress_rhs_by_grid[g] = apply_C_grid(exx, eyy, ezz, eyz, exz, exy, C)
...
def add_source_to_strain_grid_major(strain_by_grid, Q, pulse): #add the soruce
    if Q is None:
        return
...
def step_leapfrog(V, S, Q, C, rho: float, dt: float, pulse: float):
    """
    Performs one leapfrog step
    """
    strain_rhs = compute_strain_grid_major(V)

    add_source_to_strain_grid_major(strain_rhs, Q, pulse)
    stress_rhs = apply_material_grid_major(strain_rhs, C)
    update_stresses(S, stress_rhs, None, dt, pulse)
    update_velocities(V, S, rho, dt)

def total_l2(V, S):
    """
    Computes total norm for stability checks
    """
    val = 0.0
    for ci in range(4):
        for a in V[ci]:
...
def make_vti_C(rho: float, VP: float, VS: float):
    epsilon = 0.334 #hardcoded weak anisotropy parameters
    gamma = 0.575
    delta_star = 0.93
...
# ------------------------------------------------------------
# Transfrom stiff mat to stiff tensor, rotate, then transform back
# ------------------------------------------------------------
...
def rotation_x(alpha_deg: float) -> np.ndarray:
    """
    Rotation matrix:
    [[ 1, 0, 0],
     [ 0, cos a,  -sin a],
...
def rotate_stiffness(C_voigt: np.ndarray, alpha_deg: float, rot_axis: str) -> np.ndarray:
    # Rotate stiffness tensor according to section 5.6.2 in
    #https://web.archive.org/web/20090210192845/http://samizdat.mines.edu/wavesandrays/WavesAndRays.pdf
  
    if rot_axis == 'x':
        A = rotation_x(alpha_deg)
    elif rot_axis == 'y':
...
def make_tti_C(
    ...
    C_tti = rotate_stiffness(C_vti, tilt_deg, rot_axis)
    return C_tti
...
# ------------------------------------------------------------
# Christoffel curves and max wave speed
# ------------------------------------------------------------

def S_sigma(n): #normalized fourier symbol of D_sigma
    nx, ny, nz = n
    return np.array([
...
def run_Lebedev_simulation(
    ...
    if C_mat is None:
        if medium == "ISO":
            ...
            C = make_synthetic_anisotropic_C(rho, VP, VS)
        elif medium == 'ROT':
            C = make_rotated_anisotropic_C(rho,VP,VS)
        elif medium == 'TTI':
            C = make_tti_C(rho,VP,VS,epsilon,gamma,delta,tilt_deg=45, rot_axis='x')
        else:
    ...
    vmax, direction, speeds = max_wave_speed(C, rho)
    ...
    if source_type == "shear":
        Q = build_shear_source(Nx, Ny, Nz, Lx, Ly, Lz, source_width)
    elif source_type == "explosive":
        Q = build_explosive_source(Nx, Ny, Nz, Lx, Ly, Lz, source_width)
    elif source_type == 'explosive2':
        Q = build_explosive_source2(Nx, Ny, Nz, Lx, Ly, Lz, source_width)
    else:
    ...
    t0 = 1 / f0
    snapshot_time = np.array([0.45])
    snapshot_n = [int(snapshot_t / dt) for snapshot_t in snapshot_time]

    
    ###### START TIMER ######
    start = time.perf_counter()

    #Warm-up compilation happens on first step
    for n in range(Nt):
        t = n * dt
        pulse = ricker(t, f0, t0)

        step_leapfrog(V, S, Q, C, rho, dt, pulse)

        if n % 50 == 0:
            print(f"n={n:5d}/{Nt}, t={t:.4f}, norm={total_l2(V, S):.6e}")

        if n == snapshot_n[0] or n == snapshot_n[0]:
            vx_copies = []
            vy_copies = []
            vz_copies = []
            for ci in range(4):
            ...
            plot_three_planes_2d(vx_c111, Lx, Ly, Lz, None, f"x_rot{medium}_{source_type}_numbaLebedev_vxC111_t{t}N{N}.png")
            plot_three_planes_2d(vy_c111, Lx, Ly, Lz, None, f"x_rot{medium}_{source_type}_numbaLebedev_vyC111_t{t}N{N}.png")
            plot_three_planes_2d(vz_c111, Lx, Ly, Lz, None, f"x_rot{medium}_{source_type}_numbaLebedev_vzC111_t{t}N{N}.png")

            # plot_three_planes_2d(vx_c100, Lx, Ly, Lz, None, f"{medium}_{source_type}_numbaLebedev_vxC100_t{t}N{N}.png")
            # plot_three_planes_2d(vx_c010, Lx, Ly, Lz, None, f"{medium}_{source_type}_numbaLebedev_vxC010_t{t}N{N}.png")
            # plot_three_planes_2d(vx_c001, Lx, Ly, Lz, None, f"{medium}_{source_type}_numbaLebedev_vxC001_t{t}N{N}.png")
            ...
            # title=None,
            # savepath=f"{medium}_{source_type}numbastagg3d_vx_fullplanes_t{t}_N{N}.png",
            ...
            # x0=Lx/2, y0=Ly/2, z0=Lz/2,
            # title=None,
            # savepath=f"{medium}_{source_type}numbastagg3d_vy_fullplaness_t{t}_N{N}.png",
            # upsample=2, show_plane_borders=True
            # )
            # plot_lisitsa_pretty_fullplanes(
            # vz_c111,
            ...
            # x0=Lx/2, y0=Ly/2, z0=Lz/2,
            # title=None,
            # savepath=f"{medium}_{source_type}numbastagg3d_vz_fullplaness_t{t}_N{N}.png",
            # upsample=2, show_plane_borders=True
            # )
